...
original_author="ArvidQuarshie"
original_name="flasktemplate"
original_urlname="FlaskTemplate"
original_description="Awesome flasktemplate created by ArvidQuarshie" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/flasktemplate "${name}"
...
then
    ...
    echo "then run 'flasktemplate' or 'python -m flasktemplate'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'flasktemplate' or 'python -m flasktemplate' or 'poetry run flasktemplate'"
fi
