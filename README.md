...
- 🎯 Entry points to execute your program using `python -m <flasktemplate>` or `$ flasktemplate` with basic CLI argument parsing.
...
---
# flasktemplate Flask Application

Awesome flasktemplate created by ArvidQuarshie
...
```bash
git clone https://github.com/ArvidQuarshie/FlaskTemplate flasktemplate
cd flasktemplate
make install
```
...
```bash
flasktemplate create-db   # run once
flasktemplate populate-db  # run once (optional)
flasktemplate add-user -u admin -p 1234  # ads a user
flasktemplate run
```
